...
const FRAME_RATE = 2; // Slightly faster frame rate for a more "video" feel
  ...
  const startScreenSharing = async () => {
    ...
    try {
      ...
      frameIntervalRef.current = window.setInterval(() => {
        if (!ctx || !videoPreviewRef.current?.videoWidth || !sessionRef.current) return;
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          ...
          onerror: (e) => { console.error("Live Error", e); setExpression('thinking'); },
        ...
        config: {
          ...
          systemInstruction: `You are ${model}, a fun and friendly AI. Talk naturally. 
          You can see the user's screen in real-time. SEARCH THE WEB EXTENSIVELY for info.
          Comment on what you see cheerfully. IMPORTANT: Never speak URLs or mention citations.`,
          speechConfig: {
      ...
      <div className={`relative w-full max-w-5xl h-full flex flex-col items-center justify-between p-12`}>
        ...
        <div className="w-full flex justify-between items-center">
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Real-time screen review active</span>
          </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-16 relative w-full">
           {isScreenSharing ? (
             <div className="relative w-full max-w-2xl aspect-video rounded-[40px] overflow-hidden shadow-2xl border-4 border-white animate-in zoom-in-95 duration-500 group">
                ...
                <div className="absolute bottom-6 left-6 flex items-center gap-3">
                   ...
                   <div className="flex flex-col">
                      <span className="text-[10px] font-black text-white uppercase tracking-widest">Live Desktop Stream</span>
                      <span className="text-white/70 text-[10px] font-bold">AI is currently reviewing your screen</span>
                   </div>
                </div>
             </div>
           ) : (
           ...
           <div className="text-center space-y-4 max-w-lg">
              <h2 className="text-4xl font-black text-slate-900 tracking-tight">
                {expression === 'listening' ? "I'm listening!" : expression === 'happy' ? "Let's talk!" : expression === 'thinking' ? "Searching..." : "Ready"}
              </h2>
              {shareError ? (
                ...
                <p className="text-slate-400 font-semibold text-lg leading-relaxed">
                  {isScreenSharing ? "I can see everything! What should we explore?" : "Ready to chat. Ask me to see your screen!"}
                </p>
              )}
           </div>
        </div>
